//get amount calls for period by team and mail it
...
def getCallsSLAInfoUL(def team, def startTime, def endDay, def slPriority, def UL){
  ...
  if (slPriority){
    ...
    allCalls = callsIT.size() + callsPD.size()
    ...
    resolvedCalls = callsIT.size() + callsPD.size()
    ...
    resolvedQuickCalls = callsIT.size() + callsPD.size()
    ...
    atWorkQuickCalls = callsIT.size() + callsPD.size()
    ...
    atWorkNotQuickCalls = callsIT.size() + callsPD.size()
    ...
    returnCalls = callsIT.size() + callsPD.size()
...
def dateStart = '00:00:00 01.12.2018'
def dateStop = '23:59:59 31.12.2018'
def team = utils.get('team$122800201')

def report = getReport(dateStart, dateStop, team)

api.mail.sender.send(['u.user@shoko.ru':null], 'Отчет по заявкам за неделю командой ' + team?.title, report, "text/html")
